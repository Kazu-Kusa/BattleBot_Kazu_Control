...
from modules.AbsEdgeReactor import AbstractEdgeReactor, ActionPack
...
from repo.uptechStar.module.sensors import (
    SensorHub,
    LocalFullUpdaterConstructor,
    FU_INDEX,
    FullUpdater,
    IU_INDEX,
)
...
class StandardEdgeReactor(AbstractEdgeReactor):
    ...
    def register_all_config(self):
        self.register_config(
            config_registry_path=self.CONFIG_EDGE_MAX_BASELINE_KEY, value=[1900] * 4
        )
        self.register_config(
            config_registry_path=self.CONFIG_EDGE_MIN_BASELINE_KEY, value=[1300] * 4
        )
        self.register_config(
            config_registry_path=self.CONFIG_STRAIGHT_ACTION_DURATION_KEY, value=200
        )
        self.register_config(
            config_registry_path=self.CONFIG_CURVE_ACTION_DURATION_KEY, value=170
        )
        self.register_config(
            config_registry_path=self.CONFIG_BASIC_SPEED_KEY, value=4000
        )
    ...
    def __init__(
        ...
        self.updater: FullUpdater = LocalFullUpdaterConstructor.from_full_updater(
            sensor_hub.on_board_adc_updater[FU_INDEX], edge_sensor_ids
        )
        self.gray_updater: FullUpdater = (
            LocalFullUpdaterConstructor.from_indexed_updater(
                sensor_hub.on_board_io_updater[IU_INDEX], grays_sensor_ids
            )
        )
    ...
    def do_fl_rl_n_fr(self, basic_speed: int) -> ActionPack:
        ...
        return [
            ...
            new_ActionFrame(),
        ], self.DO_FL_RL_N_FR_STATUS_CODE
    ...
    def do_fl_n_rr_fr(self, basic_speed: int) -> ActionPack:
        ...
        return [
            ...
            new_ActionFrame(),
        ], self.DO_FL_N_RR_FR_STATUS_CODE
    ...
    def do_fl_rl_rr_n(self, basic_speed: int) -> ActionPack:
        return [
            ...
            new_ActionFrame(),
        ], self.DO_FL_RL_RR_N_STATUS_CODE
    ...
    def do_n_rl_rr_fr(self, basic_speed: int) -> ActionPack:
        return [
            ...
            new_ActionFrame(),
        ], self.DO_N_RL_RR_FR_STATUS_CODE
    ...
    def infer(self, edge_sensors: Sequence[int]) -> tuple[bool, ...]:
        ...
        return tuple(
            map(
                lambda pack: not (pack[0] < pack[1] < pack[2]),
                zip(min_baselines, edge_sensors, max_baselines),
            )
        )

    def react(self) -> int:
        edge_sensor_status = self.exc_action(
            self.action_table.get(self.infer(self.updater())),
            getattr(self, self.CONFIG_BASIC_SPEED_KEY),
        )
        return (
            edge_sensor_status
            if edge_sensor_status
            else self.exc_action(
                self.action_table.get(self.gray_updater()),
                getattr(self, self.CONFIG_BASIC_SPEED_KEY),
            )
        )
