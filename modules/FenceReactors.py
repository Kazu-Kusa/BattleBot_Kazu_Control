...
from modules.AbsFenceReactor import AbstractFenceReactor
...
class StandardFenceInReactor(AbstractFenceReactor):
    ...
    def register_all_config(self):
        self.register_config(
            config_registry_path=self.CONFIG_BASIC_DURATION_KEY, value=200
        )
        self.register_config(
            config_registry_path=self.CONFIG_BASIC_SPEED_KEY, value=2500
        )
        self.register_config(
            config_registry_path=self.CONFIG_LONG_TURN_DURATION_KEY, value=1000
        )

        self.register_config(
            config_registry_path=self.CONFIG_OFF_STAGE_DASH_DURATION_KEY, value=600
        )
        self.register_config(
            config_registry_path=self.CONFIG_OFF_STAGE_DASH_SPEED_KEY, value=-8000
        )

        self.register_config(
            config_registry_path=self.CONFIG_FENCE_MIN_BASE_LINE_KEY, value=[800] * 4
        )
        self.register_config(self.CONFIG_FENCE_LR_MAX_Deviation_KEY, 200)
    ...
    def __init__(
        ...
        fence_min_baselines: List[int] = getattr(
            self, self.CONFIG_FENCE_MIN_BASE_LINE_KEY
        )
        ...
        def left_right_objects_watcher() -> Tuple[bool, bool]:
            ...
            if (
                    left_data < left_sensor_fence_min_baseline
                    and right_data < right_sensor_fence_min_baseline
            ):
        ...
        self._left_right_objects_watcher: Callable[
            [], Tuple[bool, bool]
        ] = left_right_objects_watcher
    ...
    def on_front_to_fence(self, basic_speed) -> ComplexAction:
        return [
            ...
            new_ActionFrame(),
            new_ActionFrame(
                action_speed=getattr(self, self.CONFIG_OFF_STAGE_DASH_SPEED_KEY),
                action_duration=getattr(self, self.CONFIG_BASIC_DURATION_KEY),
            ),
            new_ActionFrame(),
        ]
