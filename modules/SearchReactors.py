...
from AbstractSearchReactor import AbstractSearchReactor
...
class SearchReactor(AbstractSearchReactor):
    ...
    CONFIG_SURROUNDING_WATCHER_MAX_BASELINE_KEY = (
        f"{CONFIG_SURROUNDING_WATCHER_KEY}/MaxBaseline"
    )
    CONFIG_SURROUNDING_WATCHER_MIN_BASELINE_KEY = (
        f"{CONFIG_SURROUNDING_WATCHER_KEY}/MinBaseline"
    )
    ...
    def register_all_children_config(self):
        ...
        self.register_config(
            self.CONFIG_SURROUNDING_WATCHER_MAX_BASELINE_KEY, [1900] * 4
        )
        self.register_config(
            self.CONFIG_SURROUNDING_WATCHER_MIN_BASELINE_KEY, [1500] * 4
        )
    ...
    def __init__(
        ...
        self._hall_surrounding_watcher: Watcher = watchers_merge(
            [self._extra_sensor_watcher, self._surrounding_watcher], use_any=True
        )
        ...
        self._full_edge_watcher_merged: Watcher = watchers_merge(
            [self._front_watcher_grays, self._full_edge_watcher], use_any=True
        )
        ...
        self._idle_watcher_merged: Watcher = watchers_merge(
            [self._extra_sensor_watcher, self._idle_watcher], use_any=True
        )
    ...
    def scan(self, basic_speed: int) -> ComplexAction:
        ...
        return [
            new_ActionFrame(
                action_speed=(-scanning_speed, scanning_speed),
            ...
            new_ActionFrame(),
        ] * getattr(self, self.CONFIG_SCAN_CYCLES_KEY)
    ...
    def snake(self, basic_speed: int) -> ComplexAction:
        ...
        return [
            new_ActionFrame(
                action_speed=(basic_speed, 0, basic_speed, basic_speed),
            ...
            new_ActionFrame(),
            new_ActionFrame(
                action_speed=(basic_speed, basic_speed, 0, basic_speed),
            ...
            new_ActionFrame(),
        ] * getattr(self, self.CONFIG_SNAKE_CYCLES_KEY)
    ...
    def drifting(self, basic_speed: int) -> ComplexAction:
        ...
        return [
            ...
            new_ActionFrame(
                action_speed=full_rand_drift(basic_speed),
            ...
            new_ActionFrame(),
        ] * getattr(self, self.CONFIG_DRIFTING_CYCLES_KEY)
    ...
    def plain_move(self, basic_speed: int) -> ComplexAction:
        ...
        multiplier = (
            enlarge_multiplier_l
            if self._stage_center_watcher()
            else shrink_multiplier_l
        )
        return [
            new_ActionFrame(
